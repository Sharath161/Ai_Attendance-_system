...
%=============================================================
%  MA981 template look — Orange-Book chapter style
%=============================================================
\usepackage{titlesec}
\usepackage{adjustbox}          % fit oversized figures to the page
\emergencystretch=2em           % reduce overfull lines from long unbreakable tokens

\definecolor{ocre}{RGB}{199,84,32}         % chapter / rule orange
\definecolor{covertitle}{RGB}{0,124,146}   % teal cover title
\definecolor{essexpurple}{RGB}{82,45,109}
\definecolor{essexred}{RGB}{228,0,43}

% Word rotated beside the number box; becomes APPENDIX after \appendix
\newcommand{\headword}{CHAPTER}

% Orange square holding the chapter number/letter, with rotated word to its left
\newcommand{\numberbox}[1]{%
  \begin{tikzpicture}[baseline=(num.base)]
    \node[anchor=east, inner sep=2pt] (word) at (-4pt,0)
      {\rotatebox{90}{\normalfont\scshape\large \headword}};
    \node[anchor=west, fill=ocre, minimum size=1.85cm, inner sep=0pt,
          text=white, font=\fontsize{54}{54}\selectfont\bfseries] (num) at (0,0) {#1};
  \end{tikzpicture}%
}

% Numbered chapters: right-aligned number box, orange rule, orange sans title
\titleformat{\chapter}[display]
  {\raggedright\normalfont\sffamily\bfseries\fontsize{25}{29}\selectfont\color{ocre}}
  {\hfill\numberbox{\thechapter}}
  {8pt}
  {\color{ocre}\titlerule[2.5pt]\vspace{10pt}}
\titlespacing*{\chapter}{0pt}{30pt}{34pt}

% Unnumbered chapters (Abstract, Contents, List of ..., Bibliography): rule + title
\titleformat{name=\chapter,numberless}[display]
  {\raggedright\normalfont\sffamily\bfseries\fontsize{25}{29}\selectfont\color{ocre}}
  {}
  {0pt}
  {\color{ocre}\titlerule[2.5pt]\vspace{10pt}}
\titlespacing*{name=\chapter,numberless}{0pt}{30pt}{34pt}

% Clean chapter-opening pages (no header rule / number), like the template
\fancypagestyle{plain}{\fancyhf{}\renewcommand{\headrulewidth}{0pt}}

% Essex square-mark logo (placeholder approximation — swap in the official
% logo with \includegraphics if you have the image file)
\newcommand{\essexlogo}{%
  \begin{tikzpicture}[x=0.5cm,y=0.5cm]
    \fill[essexpurple] (0,2) rectangle (1,3);
    \fill[essexpurple] (0,1) rectangle (1,2);
    \fill[essexpurple] (1,1) rectangle (2,2);
    \fill[essexred]    (1,2) rectangle (2,3);
    \fill[essexred]    (2,0) rectangle (3,1);
    \fill[essexpurple] (1,0) rectangle (2,1);
  \end{tikzpicture}%
}
...
%--- Title Page (MA981 template layout)
\begin{titlepage}
  \thispagestyle{empty}
  \setlength{\parindent}{0pt}

  % Header: Essex logo + university / department
  \begin{minipage}[c]{0.2\textwidth}
    \essexlogo
  \end{minipage}\hfill
  \begin{minipage}[c]{0.78\textwidth}
    \raggedleft
    {\LARGE\itshape University of Essex}\\[4pt]
    {\LARGE\bfseries Department of Mathematical Sciences}
  \end{minipage}

  \vspace{6pt}
  {\color{ocre}\rule{\textwidth}{2.5pt}}

  \vspace{2.4cm}
  \begin{center}
    {\LARGE MA981: D\textsc{issertation}\par}

    \vspace{1.8cm}
    {\color{covertitle}\fontsize{28}{34}\selectfont AI-Powered Automated\\[4pt]
      Attendance System\par}
    ...
    \vspace{1.8cm}
    {\Large\bfseries Sharath Chandra Ranga\par}
    \vspace{6pt}
    {\large\bfseries Student registration number: [PRN]\par}
...
The work culminates in the extraction of the recognition capability into a
standalone, domain-agnostic service, re-engineered for enrolment from very few
photographs and for execution on low-cost hardware without a GPU. Evaluated on
Labeled Faces in the Wild across 120 identities with a held-out test set, the
corrected pipeline achieves 97.5\% top-1 identification accuracy from a
\emph{single} enrolment photograph and 98.5\% from two, while rejecting
previously unseen individuals at 98.2\% detection-and-identification rate at a
1\% false-alarm rate. Enrolment amplification is shown to improve the
worst-performing class from 60\% to 80\% without materially changing the mean,
and score normalisation is shown to be the dominant factor in open-set rejection.
The chapter also documents the discovery and correction of a face-alignment
defect that had silently collapsed inter-class separation, reducing mean impostor
similarity from 0.453 to 0.012; results obtained prior to this correction are
identified as unreliable. The attendance system is rebuilt as a client of the
service, demonstrating reuse and containing no machine-learning code of its own.

\textbf{Keywords:} face recognition, ArcFace, YuNet, ESP32-CAM, attendance automation, few-shot enrolment, edge deployment, fairness in AI, microservice architecture.
...
ArcFace \cite{deng2019arcface} introduced an additive angular margin loss that maximises inter-class separability in the embedding space, achieving state-of-the-art results on LFW, MegaFace, and IJB-C benchmarks. The MobileNet backbone variant (w600k\_mbf) produces 512-dimensional L2-normalised embeddings, enabling cosine similarity matching via dot product. ArcFace outperforms the previously considered SFace (128-d) in both accuracy and demographic generalisation.
...
Multiple studies have documented demographic disparities in commercial face recognition systems. Buolamwini and Gebru \cite{buolamwini2018gender} demonstrated significantly higher error rates for darker-skinned females in commercial systems. Grother et al. \cite{grother2019frvt} (NIST FRVT) provide the most comprehensive evaluation, showing false positive rate differentials of up to 100$\times$ across demographic groups.

The FairFace dataset \cite{karkkainen2021fairface} provides 108,501 images balanced across 7 racial groups and is adopted in this work for per-group precision, recall, and F1 evaluation, with a target of F1 $\geq$ 0.75 for all groups and maximum inter-group F1 gap $\leq$ 15\%.
...
YuNet is a lightweight CNN-based face detector trained on the WIDER FACE dataset and distributed by the OpenCV Zoo project as an ONNX model (\texttt{face\_\allowbreak detection\_\allowbreak yunet\_\allowbreak 2023mar.\allowbreak onnx}, 373 KB). It outputs bounding boxes and 5-point facial landmarks (right eye, left eye, nose tip, right mouth corner, left mouth corner) with a detection score in $[0,1]$.
...
\begin{longtable}{%
  ...
  >{\footnotesize\raggedright\arraybackslash}p{1.3cm}%
  >{\footnotesize\raggedright\arraybackslash}p{4.9cm}%
  >{\footnotesize\raggedright\arraybackslash}p{2.7cm}}
...
% IoT ingestion
\texttt{/\allowbreak ingest/\allowbreak image}      & POST   & Upload JPEG from ESP32, enqueue for batch processing & Device API key \\
\texttt{/\allowbreak devices/\allowbreak register}  & POST   & Register a new ESP32 device (admin only) & Admin JWT \\
\texttt{/\allowbreak devices/\allowbreak \{id\}/\allowbreak status} & GET & Get device last-seen and status & Admin JWT \\
\midrule
% Student
\texttt{/\allowbreak students/\allowbreak register}    & POST   & Register new student with personal info & Admin JWT \\
\texttt{/\allowbreak students/\allowbreak \{id\}/\allowbreak face} & POST   & Upload face photos for embedding generation & Admin JWT \\
\texttt{/\allowbreak students/\allowbreak \{id\}/\allowbreak attendance} & GET & Full attendance history for student & Student JWT \\
\texttt{/\allowbreak students/\allowbreak \{id\}/\allowbreak attendance/\allowbreak \{module\_id\}} & GET & Module-specific attendance \% & Student JWT \\
\midrule
% Attendance
\texttt{/\allowbreak attendance/\allowbreak sessions/\allowbreak \{id\}} & GET & All attendance records for a session & Staff JWT \\
\texttt{/\allowbreak attendance/\allowbreak override}        & POST & Manually mark attendance (admin/staff) & Admin JWT \\
\texttt{/\allowbreak attendance/\allowbreak export}          & GET  & Export attendance CSV (filter by module, date) & Admin JWT \\
\midrule
% Admin / KPI
\texttt{/\allowbreak admin/\allowbreak queue/\allowbreak status}     & GET  & Queue depth, pending/done/failed counts & Admin JWT \\
\texttt{/\allowbreak admin/\allowbreak kpi/\allowbreak accuracy}     & GET  & Recognition accuracy metrics & Admin JWT \\
\texttt{/\allowbreak admin/\allowbreak kpi/\allowbreak fairness}     & GET  & Latest per-race F1 scores & Admin JWT \\
\texttt{/\allowbreak admin/\allowbreak server/\allowbreak health}    & GET  & CPU, RAM, uptime, throughput & Admin JWT \\
\midrule
% Auth
\texttt{/\allowbreak auth/\allowbreak login}    & POST & Obtain JWT token (student/staff/admin) & None \\
\texttt{/\allowbreak auth/\allowbreak refresh}  & POST & Refresh expired JWT & Refresh token \\
\end{longtable}
...
\begin{itemize}
  \item True Positive Rate (TPR): fraction of genuine pairs correctly matched
  \item False Positive Rate (FPR): fraction of impostor pairs incorrectly matched
  \item Equal Error Rate (EER): threshold at which TPR = 1 - FPR
...
\begin{itemize}
  \item Per-group F1 $\geq 0.75$ (failure to meet triggers per-race threshold adjustment)
  \item Maximum inter-group F1 gap $\leq 15\%$
\end{itemize}

\section{Performance Evaluation}
\begin{itemize}
  \item \textbf{Throughput:} number of images processed per hour during batch run on target server
  \item \textbf{CPU utilisation:} monitored via \texttt{psutil} during batch runs
  \item \textbf{Queue drain time:} time to process 1000 queued images from empty start
  \item \textbf{End-to-end latency:} from image captured at ESP32 to attendance record visible in dashboard
\end{itemize}

\section{System Reliability Evaluation}
\begin{itemize}
  \item Simulate WiFi dropout at ESP32: verify retry logic and failure logging
  \item Simulate server restart during batch: verify queue state is preserved and reprocessed
  \item Manual override audit trail: verify all overrides are logged with staff ID and timestamp
\end{itemize}
...
\begin{quote}
\textbf{Note on validity.} The measurements in this chapter were obtained on the
prototype pipeline \emph{before} the face-alignment defect described in
Section~\ref{sec:alignment-defect} was identified and corrected. That defect
suppressed inter-class separation, so the absolute similarity scores, threshold
values and derived F1 figures reported here understate the corrected system's
performance and should be read as a record of the prototype evaluation rather
than as the system's final characterisation. Definitive, post-correction results
measured on unconstrained colour imagery are reported in
Chapter~\ref{chap:api}, Section~\ref{sec:fewshot-eval}.
\end{quote}
...
%=============================================================
\chapter{From Application to Reusable API: A Few-Shot Recognition Service}
\label{chap:api}
%=============================================================

The chapters so far describe an attendance application in which recognition is an
internal component. This chapter documents the final evolution of the work: the
recognition capability was extracted into a standalone, domain-agnostic
\emph{service}, re-engineered for two deployment realities that the original
design did not address --- enrolment from very few photographs, and execution on
low-cost hardware. The attendance system was then rebuilt as a client of that
service, which serves both as a re-usability proof and as the application layer
evaluated in earlier chapters.

%-------------------------------------------------------------
\section{Motivation and Architectural Change}
\label{sec:api-motivation}
%-------------------------------------------------------------

Two limitations motivated the change. First, the recognition logic was coupled to
attendance-specific database tables, so it could not be reused for any other
purpose without carrying the entire application with it. Second, the enrolment
procedure assumed a co-operative five-angle capture session; in practice
institutions often hold only one or two usable photographs per person (for
example an existing ID-card image).

The system was therefore split into two independently deployable services
communicating over HTTP:

\begin{itemize}
  \item \textbf{Recognition service (\texttt{faceapi}).} Owns the models and the
        embedding gallery. Exposes \texttt{/enroll}, \texttt{/identify} (1:N),
        \texttt{/verify} (1:1) and \texttt{/embed}. It has no knowledge of
        students, courses or sessions.
  \item \textbf{Attendance service.} Owns all domain state --- users, courses,
        enrolments, sessions and attendance records --- and delegates every face
        operation to the recognition service. It contains no machine-learning
        code whatsoever.
\end{itemize}

This separation means the recognition service can be reused by any application,
and either service can be scaled or replaced independently. Each endpoint accepts
both \texttt{multipart/form-data} and JSON with base64-encoded images, so web,
mobile and desktop clients are supported without a bespoke SDK.

%-------------------------------------------------------------
\section{Correction of a Face-Alignment Defect}
\label{sec:alignment-defect}
%-------------------------------------------------------------

During the construction of a large-scale benchmark (Section~\ref{sec:fewshot-eval})
the pipeline returned a top-1 identification accuracy of approximately 5--10\% on
a 120-identity task where chance alone is 0.83\%. Because ArcFace is expected to
exceed 95\% on such a task, the anomaly was traced rather than reported. Two
defects were identified in the alignment and pre-processing path.

\textbf{Defect 1 --- inverted eye landmarks.} The mapping from YuNet's five
landmarks onto the canonical ArcFace template transposed the eye pair. Face
alignment is performed with \texttt{cv2.estimateAffinePartial2D}, which models
only \emph{similarity} transforms (rotation, uniform scale and translation) and
therefore cannot represent a reflection. Given an unsatisfiable correspondence,
the estimator returns a best-fit transform that distorts every face in a similar
manner. The consequence is subtle but severe: faces are still detected and
embeddings are still produced with unit norm, so no error is raised, yet the
embeddings of \emph{different} people are driven toward a common region of the
hypersphere and inter-class separation collapses.

\textbf{Defect 2 --- colour channel order.} InsightFace trains its ArcFace models
on RGB input (its reference loader sets \texttt{swapRB=True}), whereas OpenCV
supplies BGR. The unconverted crop was passed to the network.

Table~\ref{tab:alignment-fix} quantifies the correction on LFW.

\begin{table}[H]
  \centering
  \caption{Effect of the alignment and colour-order corrections, measured on LFW
  colour images. Separation is the difference between mean genuine and mean
  impostor cosine similarity.}
  \label{tab:alignment-fix}
  \begin{tabular}{lccc}
    \toprule
    \textbf{Pipeline state} & \textbf{Genuine $\mu$} & \textbf{Impostor $\mu$} & \textbf{Separation} \\
    \midrule
    Original (both defects)      & 0.516 & 0.453 & 0.063 \\
    + landmark order corrected   & 0.615 & 0.022 & 0.593 \\
    + colour order corrected     & \textbf{0.637} & \textbf{0.012} & \textbf{0.625} \\
    \bottomrule
  \end{tabular}
\end{table}

Mean impostor similarity falls from 0.453 to 0.012 --- an approximately tenfold
improvement in class separation. This has an important methodological consequence
which is stated plainly: \emph{any accuracy, threshold-calibration or fairness
figure obtained before this correction was measured on a defective pipeline}. In
particular, an earlier observation that the global threshold of 0.35 appeared
badly calibrated was an artefact of the distorted alignment rather than a property
of the model, and a unit test that had been interpreted as evidence of
threshold-related false matches was in fact detecting this defect. The results
reported in the remainder of this chapter were all produced after the correction.

The episode also illustrates a wider point about evaluating biometric pipelines:
a geometric pre-processing error produces no exception and no visibly corrupt
output, and is detectable only through an end-to-end accuracy measurement against
a known-good expectation.

%-------------------------------------------------------------
\section{Few-Shot Enrolment Techniques}
\label{sec:fewshot-methods}
%-------------------------------------------------------------

Two techniques were added to make enrolment viable when only a handful of
photographs exist.

\textbf{Enrolment amplification.} Each aligned crop is expanded into a bank of six
photometric and geometric variants --- the original, its horizontal mirror,
$\pm 8^{\circ}$ rotations, and brightness scalings of $0.85$ and $1.15$ --- and
each variant is embedded separately. A single photograph therefore yields six
prototypes, giving the matcher a degree of pose and illumination coverage it could
not otherwise obtain from one image. Matching uses the maximum cosine similarity
over a subject's stored prototypes rather than a single averaged vector, so
additional prototypes broaden coverage without diluting the class centre.

\textbf{Score normalisation (s-norm).} Raw cosine similarities are not directly
comparable between subjects: some individuals sit in denser regions of the
embedding space and consistently attract higher scores. Symmetric s-norm
standardises a score against a fixed cohort of non-enrolled identities from both
the query and the subject side:

\begin{equation}
  s' = \frac{1}{2}\left(\frac{s - \mu_q}{\sigma_q} + \frac{s - \mu_p}{\sigma_p}\right)
  \label{eq:snorm}
\end{equation}

where $(\mu_q, \sigma_q)$ and $(\mu_p, \sigma_p)$ are the mean and standard
deviation of the cohort similarities for the query embedding and the subject
prototype respectively. This equalises the operating point across enrolled
classes, which matters both for accuracy and for the equitable treatment of
individuals discussed in Chapter~\ref{chap:ethics}.

%-------------------------------------------------------------
\section{Few-Shot Evaluation on Real Faces}
\label{sec:fewshot-eval}
%-------------------------------------------------------------

Earlier chapters evaluated the pipeline on the Olivetti dataset (40 identities,
grayscale, $64\times64$) and on a small number of local enrolments. To obtain a
credible measurement, the corrected system was evaluated on
\textbf{Labeled Faces in the Wild} (LFW), which provides unconstrained colour
photographs.

\textbf{Protocol.} 4{,}324 images of 158 identities with at least ten images each
were used. The 120 identities with the most images formed the enrolled gallery;
the remaining 38 supplied both the s-norm cohort and 114 \emph{unknown} probes
that the system must reject. For each identity, $K \in \{1,2,3,5\}$ photographs
were enrolled and a fixed held-out set of five further photographs was used for
testing, so that curves across $K$ are directly comparable. Queries used flip
test-time augmentation. Alongside closed-set top-1 accuracy, the open-set
detection-and-identification rate at a 1\% false-alarm rate (DIR@FAR=1\%) was
measured, which requires the system to both reject unknown people and correctly
identify enrolled ones.

\begin{table}[H]
  \centering
  \caption{Few-shot performance on LFW (120 enrolled identities, fixed five-image
  test set per identity, 114 unknown probes). Best configuration shown:
  enrolment amplification with s-norm.}
  \label{tab:fewshot}
  \begin{tabular}{cccc}
    \toprule
    \textbf{Enrolment photos ($K$)} & \textbf{Top-1} & \textbf{Worst class} & \textbf{DIR@FAR=1\%} \\
    \midrule
    1 & 97.5\% & 0.0\%  & 97.0\% \\
    2 & 98.5\% & 80.0\% & 97.7\% \\
    3 & 98.3\% & 80.0\% & 97.8\% \\
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{fewshot_curves.png}
  \caption{Few-shot performance against the number of enrolment photographs.
  Left: closed-set top-1 accuracy. Centre: worst-class accuracy, the fairness
  floor across the 120 enrolled identities. Right: open-set DIR@FAR=1\%.}
  \label{fig:fewshot}
\end{figure}

\textbf{Low-data performance.} A single enrolment photograph is sufficient for
97.5\% top-1 accuracy across 120 identities, and two photographs raise this to
98.5\%. Accuracy then saturates: the fifth photograph adds nothing measurable to
the mean. For a university deployment this is a practical result, because it means
enrolment can be satisfied by an existing ID-card photograph rather than a
supervised capture session.

\textbf{Consistency across classes.} The mean is not the whole picture. At $K=1$
the worst-performing identity scores 0\%, meaning at least one person is never
recognised from a single photograph; the standard deviation across classes is
10.5 percentage points. At $K=2$ the worst class rises to 80\% and the spread
halves to 5.3 points. Enrolment amplification is responsible for most of this
improvement in the tail: at $K=3$ and $K=5$ it raises the worst class from 60\% to
80\% while leaving the mean essentially unchanged. The technique therefore
improves equity of treatment rather than headline accuracy, which is the more
relevant property for an attendance system where a single persistently
unrecognised student represents a real failure. \textbf{Two photographs are
consequently recommended as the practical minimum}, notwithstanding the strong
one-shot mean.

\textbf{Rejecting unknown people.} Score normalisation contributes almost nothing
to closed-set accuracy but substantially improves open-set behaviour, raising
DIR@FAR=1\% from 92.5\% to 97.7\% at $K=2$ and from 96.7\% to 98.2\% at $K=5$.
This is the security-relevant metric: a visitor who has never enrolled must not be
matched to an enrolled student.

%-------------------------------------------------------------
\section{Low-Hardware Deployment}
\label{sec:low-hardware}
%-------------------------------------------------------------

The service is designed to run without a GPU. The deployed recognition model,
ArcFace MobileNet (\texttt{w600k\_mbf}), occupies 13.6\,MB and embeds a face in
approximately 5\,ms on desktop CPU; end-to-end detection and embedding takes
approximately 10\,ms per image. Identity search is a single dense matrix--vector
product over L2-normalised embeddings and completes in under 10\,ms even with
200{,}000 enrolled prototypes, so matching is never the bottleneck.

Three deployment observations are reported, including one negative result.

\textbf{A larger backbone did not help.} Substituting ArcFace ResNet-50
(\texttt{w600k\_r50}, 174\,MB) degraded accuracy on low-resolution grayscale input
relative to the 13.6\,MB MobileNet model. The larger network is more sensitive to
the domain shift introduced by upscaled low-quality imagery. Model capacity is
therefore not automatically beneficial, and the smaller model is preferred for
constrained hardware on both efficiency and accuracy grounds.

\textbf{INT8 quantisation is a memory optimisation, not a speed optimisation.}
Dynamic INT8 quantisation reduced the model from 13.6\,MB to 3.5\,MB (a factor of
3.9) with a mean embedding cosine agreement of 0.93 against the full-precision
model. However, on the x86 test host it was approximately twelve times
\emph{slower}, because ONNX Runtime's INT8 kernels are not optimised for this
architecture. It is retained as an opt-in configuration for memory-constrained
boards, where ARM implementations commonly do show a speed benefit, with the
explicit recommendation that it be benchmarked on the target device.

\textbf{Batching benefits the embedder only.} Batched inference improves embedding
throughput by approximately 1.4$\times$ over the already multi-threaded baseline,
but detection --- which dominates end-to-end cost and is invoked through
\texttt{cv2.FaceDetectorYN} --- is not batched, so the end-to-end gain is limited.
The genuine remaining lever for constrained hardware is a quantised detector or an
accelerated execution provider.

%-------------------------------------------------------------
\section{Cross-Platform Delivery}
\label{sec:cross-platform}
%-------------------------------------------------------------

The attendance client was rebuilt as an installable Progressive Web Application
served by the attendance service. A single codebase provides camera-based
enrolment and check-in on Android, iOS and desktop, with role-aware navigation for
students, teachers and administrators and an offline application shell. This
avoids maintaining separate native clients while still supporting installation to
the device home screen. The principal constraint is that browsers restrict camera
access to secure contexts, so a deployment must be served over HTTPS.

Authentication uses JSON Web Tokens with Argon2 password hashing and three roles.
The check-in path resolves the open session for the identified student, verifies
course enrolment, blocks duplicate marks, and applies an application-level
confidence gate above the recognition service's own threshold. Correctness of the
integrated system is verified by an end-to-end test suite that starts a real
instance of the recognition service and exercises the full path from face
enrolment through check-in with held-out photographs, duplicate suppression,
rejection of unknown faces, role enforcement, manual override and report
generation.
...
The work then progressed beyond a single application. As documented in
Chapter~\ref{chap:api}, the recognition capability was extracted into a
standalone, domain-agnostic service and the attendance system rebuilt as one of
its clients, containing no machine-learning code of its own.

\section{Contributions}
The principal contributions of this dissertation are:

\begin{enumerate}
  \item \textbf{A few-shot recognition service.} A deployable API achieving 97.5\%
        top-1 accuracy across 120 identities from a \emph{single} enrolment
        photograph and 98.5\% from two, measured on unconstrained colour imagery
        with a held-out test set. This removes the requirement for a supervised
        multi-angle capture session, which is the principal practical barrier to
        deploying face-based attendance at institutional scale.
  \item \textbf{Enrolment amplification as an equity mechanism.} Expanding each
        enrolment photograph into a bank of augmented prototypes is shown to raise
        the worst-performing class from 60\% to 80\% while leaving mean accuracy
        essentially unchanged, improving consistency of treatment rather than the
        headline figure.
  \item \textbf{Empirical characterisation of open-set behaviour.} Symmetric
        s-norm score normalisation is shown to be the dominant factor in rejecting
        individuals who never enrolled, improving DIR@FAR=1\% from 92.5\% to
        97.7\% at two enrolment photographs.
  \item \textbf{Identification and correction of a silent alignment defect}
        (Section~\ref{sec:alignment-defect}) that reduced inter-class separation
        by an order of magnitude while producing no error condition, together with
        an explicit statement of which earlier results it invalidates.
  \item \textbf{Negative results for constrained deployment.} A larger ResNet-50
        backbone degraded accuracy under domain shift relative to a 13.6\,MB
        MobileNet model, and INT8 quantisation proved a memory optimisation rather
        than a speed optimisation on x86. Both are reported because they
        contradict the intuitive expectation that larger or quantised models are
        straightforwardly better.
\end{enumerate}

\section{Limitations}
Several limitations bound the claims made here. Demographic fairness results
remain calibrated against the literature rather than measured, because the
operational database contains no self-reported demographic labels; a measured
per-group evaluation requires a labelled dataset such as FairFace and is the most
important outstanding piece of validation. One-shot enrolment, although strong on
average, leaves at least one identity unrecognised in the 120-class benchmark, so
two photographs are recommended in practice. The system performs no liveness
detection and is therefore vulnerable to printed-photograph and screen-replay
attacks. Finally, the low-hardware configuration was benchmarked on x86; the
INT8 path in particular requires re-measurement on the target ARM device.

\section{Future Work}
\begin{enumerate}
  \item \textbf{Measured fairness evaluation:} run the corrected pipeline against a
        demographically labelled dataset (FairFace) to replace the
        literature-calibrated fairness figures with measured per-group results
  \item \textbf{Liveness detection:} integrate passive liveness (blink, depth, texture analysis) to prevent printed-photo spoofing
  \item \textbf{Quantised detector:} detection dominates end-to-end latency; an INT8 or accelerated YuNet is the remaining substantial efficiency gain for edge hardware
...
\appendix
\renewcommand{\headword}{APPENDIX}
